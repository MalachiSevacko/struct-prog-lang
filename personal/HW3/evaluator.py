...
# --- Debug watch homework ---
WATCH_IDENTIFIER = None          # name of variable to watch 
WATCH_LOCATION_MAP = {}          # maps id(ast_node) -> {"line": int, "position": int}

def build_location_map(ast, tokens):

    location_map = {}
    index = 0  # index into token list

    def match_leaf(node):
        nonlocal index
        tag = node["tag"]

        # Only leaf nodes that come directly from tokens
        if tag in ["identifier", "number", "string", "boolean", "null"]:
            # Scan forward for the next token with the same tag AND value
            while index < len(tokens):
                tok = tokens[index]
                index += 1
                if tok["tag"] != tag:
                    continue
                # For tags that carry a value, make sure it matches
                if "value" in node:
                    if tok.get("value") != node["value"]:
                        continue
                # Found a match
                location_map[id(node)] = {
                    "line": tok.get("line"),
        ...
        # Composite nodes: recurse through children in a consistent order
        if tag in ["+", "-", "*", "/", "%", "&&", "||", "and", "or",
                   "<", ">", "<=", ">=", "==", "!="]:
            match_leaf(node["left"])
            match_leaf(node["right"])
        elif tag in ["negate", "not", "!"]:
            match_leaf(node["value"])
        elif tag == "list":
            ...
            for item in node["items"]:
                match_leaf(item["key"])
                match_leaf(item["value"])
        elif tag == "complex":
            match_leaf(node["base"])
            match_leaf(node["index"])
        elif tag == "assign":
            match_leaf(node["target"])
            match_leaf(node["value"])
        elif tag == "print":
            if node.get("value") is not None:
                match_leaf(node["value"])
        elif tag == "assert":
            match_leaf(node["condition"])
            if "explanation" in node and node["explanation"] is not None:
                match_leaf(node["explanation"])
        elif tag == "if":
            match_leaf(node["condition"])
            match_leaf(node["then"])
            if "else" in node:
                match_leaf(node["else"])
        elif tag == "while":
            match_leaf(node["condition"])
            match_leaf(node["do"])
        elif tag in ["statement_list", "program"]:
            for st in node["statements"]:
                match_leaf(st)
        elif tag == "function":
            match_leaf(node["body"])
        elif tag == "call":
            match_leaf(node["function"])
            for arg in node["arguments"]:
                match_leaf(arg)
        elif tag in ["return", "exit"]:
            if node.get("value") is not None:
                match_leaf(node["value"])
...
def evaluate(ast, environment):
    ...
    if ast["tag"] == "assign":
        ...
        # --- WATCH SUPPORT: print when this identifier changes ---
        from evaluator import WATCH_IDENTIFIER, WATCH_LOCATION_MAP  # safe self-import

        if (
            WATCH_IDENTIFIER is not None
            and target["tag"] == "identifier"
            and target["value"] == WATCH_IDENTIFIER
        ):
            loc = WATCH_LOCATION_MAP.get(id(target))
            if loc is not None and loc.get("line") is not None:
                where = f"line {loc['line']}"
            else:
                where = "unknown location"
            print(f"[watch] {WATCH_IDENTIFIER} = {value} ({where})")
