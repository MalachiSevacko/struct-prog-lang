...
def parse_assignment_expression(tokens):
    ...
    # Handle normal and augmented assignment
    if op in ["=", "+=", "-=", "*=", "/=", "%="]:
        tokens = tokens[1:]
        ...
        # Desugar augmented assignment into a normal assign
        if op == "=":
            value_ast = right
        else:
            base_op = {
        ...
        return {"tag": "assign", "target": left, "value": value_ast}, tokens
...
def test_parse_assignment_expression():
    ...
    ast, tokens = parse_assignment_expression(tokenize("x *= y"))
    assert ast == {
        "tag": "assign",
        "target": {"tag": "identifier", "value": "x"},
        "value": {
    ...
    ast, tokens = parse_assignment_expression(tokenize("x /= 2"))
    assert ast == {
        "tag": "assign",
        "target": {"tag": "identifier", "value": "x"},
        "value": {
